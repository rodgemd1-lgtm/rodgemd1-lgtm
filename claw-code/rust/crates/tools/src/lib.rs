...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            ...
            description: "Read a specific resource from an MCP server by server name and URI.",
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolSpec {
            ...
            description: "Ask the user a question during execution to gather preferences, clarify instructions, or offer choices. Returns the user's response.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "questions": {
        ...
        ToolSpec {
            name: "CronCreate",
            description: "Schedule a prompt to run on a cron schedule. Session-scoped, does not persist across restarts.",
            input_schema: json!({
                "type": "object",
                "properties": {
        ...
        ToolSpec {
            name: "CronList",
            description: "List all cron jobs scheduled in this session.",
            input_schema: json!({
                "type": "object",
                "properties": {},
                "additionalProperties": false
...
pub fn execute_tool(name: &str, input: &Value) -> Result<String, String> {
    match name {
        ...
        // PR 1: MCP Resource tools (require MCP context - routed via GlobalToolRegistry)
        "ListMcpResources" | "ReadMcpResource" => {
            Err(format!("{name} requires MCP server manager context; route through GlobalToolRegistry.execute()"))
        }
        // PR 2: AskUserQuestion (requires interactive prompter - routed via conversation runtime)
        "AskUserQuestion" => {
            Err("AskUserQuestion requires interactive user prompter; route through conversation runtime".to_string())
        }
        // PR 3: Scheduling tools (require session scheduler - routed via extended executor)
        "CronCreate" | "CronDelete" | "CronList" => {
            Err(format!("{name} requires session scheduler context; route through extended executor"))
        }
