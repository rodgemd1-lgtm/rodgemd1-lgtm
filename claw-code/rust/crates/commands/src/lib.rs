...
pub fn handle_slash_command(
    ...
    match command {
        ...
        SlashCommand::Bughunter { scope } => {
            ...
            let message = handlers::handle_bughunter_command(scope.as_deref(), &cwd)
                .unwrap_or_else(|e| format!("bughunter error: {e}"));
            Some(SlashCommandResult {
                message,
                session: session.clone(),
            })
        ...
        SlashCommand::Ultraplan { task } => {
            let cwd = env::current_dir().unwrap_or_default();
            let message = handlers::handle_ultraplan_command(task.as_deref(), &cwd)
                .unwrap_or_else(|e| format!("ultraplan error: {e}"));
            Some(SlashCommandResult {
                message,
                session: session.clone(),
            })
        ...
        SlashCommand::Teleport { target } => {
            let cwd = env::current_dir().unwrap_or_default();
            let message = handlers::handle_teleport_command(target.as_deref(), &cwd)
                .unwrap_or_else(|e| format!("teleport error: {e}"));
            Some(SlashCommandResult {
                message,
                session: session.clone(),
            })
